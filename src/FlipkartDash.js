...
setInterval(()=>{
  useEffect(() => {
    const getDetails = async () => {
      try {
        const res = await fetch("https://webscrpe.herokuapp.com/flipkart", {
          method: "GET",
          headers: {
  ...
  const editPrice = async () => {
    try {
      const response = await fetch(
        `https://webscrpe.herokuapp.com/flipkart/${id}`,
        {
          method: "PUT",
          body: JSON.stringify({
            buyPrice: price,
  ...
  const handleDelete = async (item) => {
    try {
      ...
      const res = await fetch(
        `https://webscrpe.herokuapp.com/flipkart/${idx}`,
        {
          method: "DELETE",
          headers: {
            "x-auth-token": localStorage.getItem("token"),
  ...
  return (
    <div className="product-page">
      ...
      <h1>Enter details to scrape Flipkart product</h1>
      <main>
